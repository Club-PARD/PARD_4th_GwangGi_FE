...
function QPage12() {
    ...
    const handleSubmit = async () => {
        ...
        // 상태 업데이트 후 일정 시간 대기하여 상태가 업데이트된 후에 서버로 전송
        setTimeout(async () => {
            const formData = {
                ...state,
            ...
            // 서버로 전송
            try {
                const response = await axios.post(`${server}api/questionnaire`, formData, {
                ...
                navigate('/t_agree', { state: { responseData: response.data } });
            } catch (error) {
